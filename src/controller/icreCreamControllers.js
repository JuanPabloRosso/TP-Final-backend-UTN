...
const createIceCream = async (req, res) => {
    ...
    try {
        ...
        const newIceCream = await iceCreamModels.createIceCream({name, price, description, stock, expiredAt});

        console.log(newIceCream);
        res.status(201).json(newIceCream)
    } catch (error) {
        res.status(500).json({message: "server error: error when adding ice cream"})
    }
}
...
export {getAllIceCream, getIceCreamById, createIceCream, updateIceCream, deleteIceCream, getIceCreamExpired} 